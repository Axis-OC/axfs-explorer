...
sealed class SoundManager : IDisposable
{
    ...
    void EnsureBeepFile()
    {
        if (_beep1000Path != null && File.Exists(_beep1000Path)) return;
        try
        {
        ...
        catch { _beep1000Path = null; }
    }

    public void StartAmbient()
    {
        if (EffGen <= 0.001) return;
        StopAmbient();
        ...
        catch { _ambientPlayer = null; }
    }

    public void StopAmbient()
    {
        try { _ambientPlayer?.Pause(); _ambientPlayer?.Dispose(); } catch { }
        _ambientPlayer = null;
    }
    ...
    {
        StopAmbient();
        try { if (_beep1000Path != null) File.Delete(_beep1000Path); } catch { }
    }
}